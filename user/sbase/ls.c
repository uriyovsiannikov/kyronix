...
static void mkent(struct entry* ent, char* path, int dostat, int follow)
{
    struct stat st;
...
static char* indicator(mode_t mode)
{
    if (pflag || Fflag)
        if (S_ISDIR(mode))
            return "/";
...
static void output(const struct entry* ent)
{
    struct group* gr;
    struct passwd* pw;
    struct tm* tm;
    ssize_t len;
    char *fmt, buf[BUFSIZ], pwname[_SC_LOGIN_NAME_MAX], grname[_SC_LOGIN_NAME_MAX],
        mode[] = "----------";
...
static int entcmp(const void* va, const void* vb)
{
    int cmp = 0;
...
static void lsdir(const char* path, const struct entry* dir)
{
    ...
    while ((d = readdir(dp)))
    {
        ...
        mkent(&ents[n - 1], estrdup(d->d_name), Fflag || iflag || lflag || pflag || Rflag || sort,
              Lflag);
    }
    ...
    if (Rflag)
    {
        if (snprintf(prefix, PATH_MAX, "%s%s/", path, dir->name) >= PATH_MAX)
...
static int visit(const struct entry* ent)
{
    dev_t dev;
    ino_t ino;
    int i;
...
static void ls(const char* path, const struct entry* ent, int listdir)
{
    int treeind;
    char cwd[PATH_MAX];
...
int main(int argc, char* argv[])
{
    ...
    case 1:
        ...
        ls("", &ent,
           (!dflag && S_ISDIR(ent.mode)) ||
               (S_ISLNK(ent.mode) && S_ISDIR(ent.tmode) && !(dflag || Fflag || lflag)));
