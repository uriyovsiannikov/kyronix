...
static int64_t sys_brk(uint64_t addr) {
    ...
    if (addr == 0 || addr <= p->brk) return (int64_t)p->brk;
...
static int64_t sys_mmap(uint64_t addr, uint64_t length, uint64_t prot,
    ...
    proc_t *p = cur();
    if (!p || !p->space) return -(int64_t)ENOMEM;
    if (!(flags & MAP_ANON)) return -(int64_t)ENOSYS;
    ...
    for (uint64_t o = 0; o < length; o += PAGE_SIZE) {
        ...
        if (vmm_map(p->space, va + o, (uint64_t)ph, vf) < 0) {
            pmm_free(ph); return -(int64_t)ENOMEM;
        }
    }
    return (int64_t)va;
}
...
#define FREE_EXEC_STRS() do { \
    for (int _i=0;_i<MAX_EXEC_ARGS;_i++) { \
        if (argv_mem[_i]) { kfree(argv_mem[_i]); argv_mem[_i]=NULL; } \
        if (envp_mem[_i]) { kfree(envp_mem[_i]); envp_mem[_i]=NULL; } \
    } } while(0)
    ...
    if (uargv) {
        while (argc < MAX_EXEC_ARGS && uargv[argc]) {
            ...
            if (!argv_mem[argc]) { FREE_EXEC_STRS(); return -(int64_t)ENOMEM; }
    ...
    if (argc == 0) {
        ...
        if (!argv_mem[0]) { FREE_EXEC_STRS(); return -(int64_t)ENOMEM; }
    ...
    if (uenvp) {
        while (envc < MAX_EXEC_ARGS && uenvp[envc]) {
            ...
            if (!envp_mem[envc]) { FREE_EXEC_STRS(); return -(int64_t)ENOMEM; }
    ...
    uint64_t rsp = setup_user_stack(res.space, &res, argc,
                                     (const char * const *)kargv,
                                     (const char * const *)kenvp);
    FREE_EXEC_STRS();

    if (!rsp) { vmm_space_free(res.space); return -(int64_t)ENOMEM; }
    ...
    while (1) {
        ...
        for (int i = 0; i < PROC_MAX; i++) {
            ...
            if (c->state == PROC_ZOMBIE) {
                ...
                uint32_t cpid = c->pid;
                if (c->fds) vfs_free_fdtable(c->fds);
...
static int64_t sys_getpgrp(void) { return cur() ? (int64_t)cur()->pid : 1; }
static int64_t sys_setsid(void)  { return cur() ? (int64_t)cur()->pid : 1; }
static int64_t sys_setpgid(uint64_t p, uint64_t g) { (void)p; (void)g; return 0; }
...
static int64_t sys_prlimit64(uint64_t p, uint64_t r, void *nl, void *ol) {
    (void)p; (void)r; (void)nl;
    if (ol) { ((uint64_t*)ol)[0]=(uint64_t)-1; ((uint64_t*)ol)[1]=(uint64_t)-1; }
    return 0;
}

static int64_t sys_rt_sigaction(int sig, const k_sigaction_t *act,
                                 k_sigaction_t *oldact, uint64_t sigsetsize) {
    (void)sigsetsize;
...
static int64_t sys_rt_sigprocmask(int how, const uint64_t *set,
    ...
    switch (how) {
        case SIG_BLOCK:    p->sig_mask |=  bits; break;
        case SIG_UNBLOCK:  p->sig_mask &= ~bits; break;
        case SIG_SETMASK:  p->sig_mask  =  bits; break;
...
static int64_t sys_nanosleep(void *r, void *m) { (void)r; (void)m; return 0; }
static int64_t sys_set_tid_address(void *p) { (void)p; return sys_getpid(); }
static int64_t sys_set_robust_list(void *h, uint64_t l) { (void)h; (void)l; return 0; }

#define FUTEX_WAIT          0
#define FUTEX_WAKE          1
#define FUTEX_PRIVATE_FLAG  128
static int64_t sys_futex(uint32_t *uaddr, int op, uint32_t val,
                          void *timeout, uint32_t *uaddr2, uint32_t val3) {
    (void)timeout; (void)uaddr2; (void)val3;
    if (!uaddr) return -(int64_t)EFAULT;
...
static int64_t sys_prctl(int op, uint64_t a2, uint64_t a3, uint64_t a4, uint64_t a5) {
    (void)op; (void)a2; (void)a3; (void)a4; (void)a5;
    return 0;
}
...
static int64_t sys_ppoll(struct pollfd_s *fds, uint64_t nfds,
                          void *tmo, const void *sigmask, uint64_t sigsetsize) {
    (void)tmo; (void)sigmask; (void)sigsetsize;
    return sys_poll(fds, nfds, 0);
}

static int64_t sys_select(int nfds, void *rfds, void *wfds, void *efds,
                           void *timeout) {
    (void)nfds; (void)rfds; (void)wfds; (void)efds; (void)timeout;
    return 0;
}

static int64_t sys_pselect6(int nfds, void *rfds, void *wfds, void *efds,
                              void *timeout, void *sigmask) {
    (void)nfds; (void)rfds; (void)wfds; (void)efds;
    (void)timeout; (void)sigmask;
    return 0;
}

static int64_t sys_umask(uint64_t mask) { (void)mask; return 0022; }

static int64_t sys_ftruncate(int fd, uint64_t len) { (void)fd; (void)len; return 0; }
...
static int64_t sys_getitimer(int w, void *v) { (void)w; if(v) memset(v,0,32); return 0; }
...
static int64_t sys_clock_gettime(uint64_t c, void *t) {
    (void)c;
    if (t) { ((uint64_t*)t)[0]=0; ((uint64_t*)t)[1]=0; }
    return 0;
}

static int64_t sys_gettimeofday(void *tv, void *tz) {
    (void)tz;
    if (tv) { ((uint64_t*)tv)[0]=0; ((uint64_t*)tv)[1]=0; }
    return 0;
}

static int64_t sys_times(void *b) { if (b) memset(b, 0, 32); return 0; }
...
void syscall_dispatch(syscall_frame_t *f) {
    ...
    int64_t ret;
    switch (nr) {
        case   0: ret = fd_read   ((int)a1, (void*)a2, a3);                    break;
        case   1: ret = fd_write  ((int)a1, (const void*)a2, a3);              break;
        case   2: ret = fd_open   ((const char*)a1, (int)a2, (int)a3);         break;
        case   3: ret = fd_close  ((int)a1);                                    break;
        case   4: ret = fd_stat   ((const char*)a1, (struct linux_stat*)a2);   break;
        case   5: ret = fd_fstat  ((int)a1, (struct linux_stat*)a2);           break;
        case   6: ret = fd_lstat  ((const char*)a1, (struct linux_stat*)a2);   break;
        case   7: ret = sys_poll  ((struct pollfd_s*)a1, a2, (int)a3);         break;
        case   8: ret = fd_lseek  ((int)a1, (int64_t)a2, (int)a3);            break;
        case   9: ret = sys_mmap  (a1, a2, a3, a4, a5, a6);                   break;
        case  10: ret = sys_mprotect(a1, a2, a3);                             break;
        case  11: ret = sys_munmap(a1, a2);                                    break;
        case  12: ret = sys_brk   (a1);                                        break;
        case  13: ret = sys_rt_sigaction((int)a1,(const k_sigaction_t*)a2,
                                         (k_sigaction_t*)a3, a4);             break;
        case  14: ret = sys_rt_sigprocmask((int)a1,(const uint64_t*)a2,
                                           (uint64_t*)a3, a4);                break;
        case  15: ret = sys_rt_sigreturn(f);                                   break;
        case  16: ret = fd_ioctl  ((int)a1, a2, a3);                          break;
        case  17: { int64_t s = fd_lseek((int)a1,(int64_t)a4,SEEK_SET);
                    ret = (s < 0) ? s : fd_read((int)a1,(void*)a2,a3);        break; }
        case  19: ret = sys_readv((int)a1, (const struct iovec*)a2, (int)a3); break;
        case  20: ret = sys_writev((int)a1, (const void*)a2, (int)a3);        break;
        case  21: ret = sys_access((const char*)a1, (int)a2);                 break;
        case  22: ret = fd_pipe   ((int*)a1);                                  break;
        case  23: ret = sys_select((int)a1,(void*)a2,(void*)a3,
                                   (void*)a4,(void*)a5);                      break;
        case  28: ret = sys_madvise((void*)a1, a2, (int)a3);                  break;
        case  32: ret = fd_dup    ((int)a1);                                   break;
        case  33: ret = fd_dup2   ((int)a1, (int)a2);                         break;
        case  35: ret = sys_nanosleep((void*)a1, (void*)a2);                  break;
        case  36: ret = sys_getitimer((int)a1, (void*)a2);                    break;
        case  38: ret = sys_setitimer((int)a1,(const void*)a2,(void*)a3);     break;
        case  39: ret = sys_getpid();                                          break;
        case  57: ret = sys_fork  (f);                                         break;
        case  59: ret = sys_execve((const char*)a1,(const char**)a2,
                                   (const char**)a3);                         break;
        case  60: proc_do_exit((int)a1); return;
        case  61: ret = sys_wait4 ((int)a1,(int*)a2,(int)a3,(void*)a4);       break;
        case  62: ret = sys_kill  (a1, (int)a2);                              break;
        case  63: ret = sys_uname ((struct utsname*)a1);                      break;
        case  72: ret = fd_fcntl  ((int)a1, (int)a2, a3);                     break;
        case  77: ret = sys_ftruncate((int)a1, a2);                           break;
        case  78: ret = fd_getdents64((int)a1, (void*)a2, a3);                break;
        case  79: ret = sys_getcwd((char*)a1, a2);                            break;
        case  80: ret = sys_chdir ((const char*)a1);                          break;
        case  89: ret = fd_readlink((const char*)a1, (char*)a2, a3);          break;
        case  95: ret = sys_umask (a1);                                        break;
        case  96: ret = sys_gettimeofday((void*)a1, (void*)a2);               break;
        case  97: ret = sys_getrlimit(a1, (void*)a2);                         break;
        case 100: ret = sys_times ((void*)a1);                                 break;
        case 102: ret = sys_getuid();                                          break;
        case 104: ret = sys_getgid();                                          break;
        case 105: ret = sys_setgroups((int)a1, (uint32_t*)a2);                break;
        case 107: ret = sys_geteuid();                                         break;
        case 108: ret = sys_getegid();                                         break;
        case 109: ret = sys_setpgid(a1, a2);                                  break;
        case 110: ret = sys_getppid();                                         break;
        case 111: ret = sys_getpgrp();                                         break;
        case 112: ret = sys_setsid();                                          break;
        case 115: ret = sys_getgroups((int)a1,(uint32_t*)a2);                 break;
        case 121: ret = sys_getpgrp();   /* getpgid */                         break;
        case 131: ret = sys_sigaltstack((const void*)a1, (void*)a2);          break;
        case 137: ret = sys_statfs ((const char*)a1, (void*)a2);              break;
        case 138: ret = sys_statfs (NULL, (void*)a2);                         break;
        case 157: ret = sys_prctl  ((int)a1, a2, a3, a4, a5);                 break;
        case 158: ret = sys_arch_prctl((int)a1, a2);                          break;
        case 160: ret = sys_getrlimit(a1, (void*)a2);                         break;
        case 186: ret = sys_gettid();                                          break;
        case 200: ret = sys_kill  ((int64_t)a1, (int)a2); break;  /* tkill */
        case 202: ret = sys_futex  ((uint32_t*)a1,(int)a2,(uint32_t)a3,
                                    (void*)a4,(uint32_t*)a5,(uint32_t)a6);    break;
        case 217: ret = fd_getdents64((int)a1, (void*)a2, a3);                break;
        case 218: ret = sys_set_tid_address((void*)a1);                       break;
        case 228: ret = sys_clock_gettime(a1, (void*)a2);                     break;
        case 229: ret = 0;                                                     break;
        ...
        case 271: ret = sys_ppoll  ((struct pollfd_s*)a1,a2,
                                    (void*)a3,(const void*)a4,a5);            break;
        case 273: ret = sys_set_robust_list((void*)a1, a2);                   break;
        case 292: ret = fd_dup2   ((int)a1, (int)a2);                         break;
        case 293: ret = fd_pipe   ((int*)a1);                                  break;
        case 302: ret = sys_prlimit64(a1,a2,(void*)a3,(void*)a4);             break;
