...
int vmm_map(vmm_space_t *sp, uint64_t virt, uint64_t phys, uint64_t flags) {
    ...
    uint64_t *pdpt = descend(pml4, PML4_IDX(virt));  if (!pdpt) return -1;
    uint64_t *pd   = descend(pdpt, PDPT_IDX(virt));  if (!pd)   return -1;
    uint64_t *pt   = descend(pd,   PD_IDX(virt));    if (!pt)   return -1;

    pt[PT_IDX(virt)] = (phys & PTE_ADDR_MASK)
...
void vmm_space_free(vmm_space_t *sp) {
    ...
    for (int i = 0; i < VMM_MAX_SPACES; i++) {
        if (&g_pool[i] == sp) { g_pool_used[i] = false; break; }
    }
}
